...
class PDAN(nn.Module):
    def __init__(
        ...
        self.stages = nn.ModuleList(
            [
                copy.deepcopy(SSPDAN(num_layers, num_f_maps, num_classes, num_classes))
...
class SSPDAN(nn.Module):
    def __init__(self, num_layers, num_f_maps, dim, num_classes):
        ...
        self.layers = nn.ModuleList(
            [
                copy.deepcopy(PDAN_Block(2**i, num_f_maps, num_f_maps))
...
class PDAN_Block(nn.Module):
    def __init__(self, dilation, in_channels, out_channels):
        ...
        self.conv_attention = DAL(
            in_channels, out_channels, kernel_size=3, padding=dilation, dilated=dilation
        )
...
class DAL(nn.Module):
    def __init__(
        ...
        self.rel_t = nn.Parameter(
            torch.randn(out_channels, 1, kernel_size), requires_grad=True
        )
    ...
    def forward(self, x):
        ...
        k_out = torch.cat(
            (
                k_out[:, :, :, 0].unsqueeze(3),
                k_out[:, :, :, 0 + self.dilated].unsqueeze(3),
                k_out[:, :, :, 0 + 2 * self.dilated].unsqueeze(3),
            ),
            dim=3,
        )  # dilated
        ...
        v_out = torch.cat(
            (
                v_out[:, :, :, 0].unsqueeze(3),
                v_out[:, :, :, 0 + self.dilated].unsqueeze(3),
                v_out[:, :, :, 0 + 2 * self.dilated].unsqueeze(3),
            ),
            dim=3,
        )  # dilated
        v_out = v_out + self.rel_t
        k_out = k_out.contiguous().view(
            batch, self.groups, self.out_channels // self.groups, time, -1
        )
