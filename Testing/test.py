...
if args.dataset == "TSU":
    ...
    # rgb_root = "C:/Users/angyi/Documents/SIT Year 3/3104/Project/TSU_RGB_i3d_feat/RGB_i3d_16frames_64000_SSD"
    rgb_root = "C:/Users/jaspz/Documents/GitHub/ict3104-team10-2022/Training/RGB_i3d_16frames_64000_SSD"
...
def load_data_rgb_skeleton(val_split, root_skeleton, root_rgb):
    ...
    val_dataset = Dataset(
        val_split, "testing", root_skeleton, root_rgb, batch_size, classes
    )
...
def run(models, criterion, num_classes):
    ...
    for model, gpu, dataloader, optimizer, sched, model_file in models:
        prob_val, val_loss, val_map = val_step(
            model, gpu, dataloader["val"], num_classes
        )
...
def val_step(model, gpu, dataloader, classes):
        ...
        for event in predicted_event:
            ...
            events.append(
                [
                    ...
                    vid_name,
                    (100 * probs.data.cpu().numpy()[0][num_iter][event]),
                ]
            )
    ...
    merge_output = pd.merge(
        activity_names, prob_values, on="Activity_Index", how="outer"
    )
    ...
    print(
        ...
        + "\033[1m"
        + "\u2713"
        + " results exported to: ./Testing/results/results.csv"
        + "\033[0m"
    )
    ...
    # Generate table
    table = wandb.Table(
        data=final_output, columns=["Activity", "Average Class Prediction"]
    )

    # Plot graphs
    wandb.log(
        {
            "Average Class Prediction": wandb.plot.bar(
                table, "Activity", "Average Class Prediction"
            )
        }
    )
    wandb.log(
...
if __name__ == "__main__":
    ...
    if args.test:
        ...
        run(
            [(model, 0, dataloaders, optimizer, lr_sched, args.comp_info)],
            criterion,
            num_classes=classes,
        )
