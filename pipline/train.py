...
def run(models, criterion, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        for model, gpu, dataloader, optimizer, sched, model_file in models:
            ...
            if best_map < val_map:
                best_map = val_map
                torch.save(model.state_dict(),'./pipline/'+str(args.model)+'/weight_epoch_'+str(args.lr)+'_'+str(epoch))
                torch.save(model,'./pipline/'+str(args.model)+'/model_epoch_'+str(args.lr)+'_'+str(epoch))
                print('save here:','./pipline/'+str(args.model)+'/weight_epoch_'+str(args.lr)+'_'+str(epoch))
